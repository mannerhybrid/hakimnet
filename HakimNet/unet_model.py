...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr  3 14:21:26 2018

@author: user
"""
import unet, nifti_segmenter
import numpy as np
import keras.backend as K
from keras.models import Model
from keras.layers import Input, Conv3D, Conv3DTranspose,UpSampling2D, MaxPooling3D, UpSampling3D, Concatenate
from keras import optimizers as opt
from unet import form_dataset_from_CSV
...
def unet_model(input_image_size,
    ...
    for i in range(len(layers)):
        ...
        encoding_convolution_layers.append(Conv3D(filters=number_of_filters, 
                                                            ...
                                                            padding='same')(conv))
        if i < len(layers)-1:
            pool = MaxPooling3D(pool_size=pool_size, data_format="channels_first")(encoding_convolution_layers[i])
    ...
    for i in range(1,len(layers)):
        ...
        tmp_deconv = UpSampling3D(size=pool_size, data_format="channels_first")(tmp_deconv)
        ...
        if mode == 'classification':
            if number_of_classification_labels == 1:
                outputs = Conv3D(filters=number_of_classification_labels, kernel_size=(1,1,1), activation='sigmoid')(outputs)
            ...
            outputs = Conv3D(filters=number_of_classification_labels, kernel_size=(1,1), 
...
model = unet_model(input_image_size=(1,50,50,50))